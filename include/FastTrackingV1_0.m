function [trackResults, channel]= FastTrackingV1_0(fid, channel, settings)
...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        caCode = [caCode0(1023) caCode0 caCode0(1)];
        caCode = caCode>0;
...
%                 end
            ...
            iBasebandSignal = (rawSignal2 .* carrSin) + (rawSignal1 .* carrCos);
            qBasebandSignal = (rawSignal2 .* carrCos) - (rawSignal1 .* carrSin);

            % Now get early, late, and prompt values for each
            I_E = sum(iBasebandSignal(earlyCode))-sum(iBasebandSignal(~earlyCode));
            Q_E = sum(qBasebandSignal(earlyCode))-sum(qBasebandSignal(~earlyCode));
            I_P = sum(iBasebandSignal(promptCode))-sum(iBasebandSignal(~promptCode));
            Q_P = sum(qBasebandSignal(promptCode))-sum(qBasebandSignal(~promptCode));
            I_L = sum(iBasebandSignal(lateCode))-sum(iBasebandSignal(~lateCode));
            Q_L = sum(qBasebandSignal(lateCode))-sum(qBasebandSignal(~lateCode));
