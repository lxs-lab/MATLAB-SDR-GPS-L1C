...
fprintf('\n|=========|=====|===============|===========|=============|============|========|\n');
fprintf(  '| Channel | PRN |   Frequency   |  Doppler  | Code Offset | Time Delay | Status |\n');
fprintf(  '|_________|_____|_______________|___________|_____________|____________|________|\n');

for channelNr = 1 : settings.numberOfChannels
    if (channel(channelNr).status ~= '-')
        fprintf('|      %02d | %03d |  %+2.5e |   %+5.0f   |    %06d   | %+8.7f |     %1s  |\n', ...
                channelNr, ...
                channel(channelNr).PRN, ...
                channel(channelNr).acquiredFreq, ...
                channel(channelNr).acquiredFreq - settings.IF, ...
                channel(channelNr).codePhase, ...
                channel(channelNr).codePhase/settings.samplingFreq*1e3,...
