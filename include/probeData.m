...
if (fid > 0)
   ...
   [data, count] = fread(fid, [1, dataAdaptCoeff*100*samplesPerCode], settings.dataType);
    ...
    sc = sc(3:4);
    picSize = [1000,1000];
    while sc(1)<picSize(1) && sc(2)<picSize(2)
        picSize = round(picSize*0.8);
    end
    StartPos = round((sc-picSize)/2);
    fig = figure('Color','w','Position',[StartPos,picSize]);
    clf(fig);
    ...
    if (settings.fileType == 1)
        ...
        ax = subplot(3, 2, 6);
        [edge,~] = calXPdf(ax,real(data));
        dmax = edge(2) + 1;
        axis tight;     adata = axis;
        axis([-dmax dmax adata(3) adata(4)]);
        grid on;        title ('Histogram (I)');
        xlabel('Sample''s amptitude');  ylabel('Prob');

        ax = subplot(3, 2, 5);
        [edge,~] = calXPdf(ax,imag(data));
        dmax = edge(2) + 1;
        axis tight;     adata = axis;
        axis([-dmax dmax adata(3) adata(4)]);
