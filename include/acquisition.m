...
% debug Switch (plot acq function for each satellites)
% add by xiaoyeyimier
debugPlot = true;
...
% Initialization for plotting acq function for each satellites
% add by xiaoyeyimier
if debugPlot
    SC = get(0,'ScreenSize');
    SC(1:2) = [100,100];
    SC(3:4) = SC(3:4)-[200,200];
    Dfig = figure('Position',SC,'color','w');
    ax_arr = cell(32,1);
    for ax_i = 1:4
        for ax_j = 1:8
            ax_arr{8*(ax_i-1)+ax_j} = subplot(4,8,8*(ax_i-1)+ax_j);
        end
    end
    debugTimeAxes = 1e3*(1:samplesPerCode*2)/settings.samplingFreq; % time delay [ms]
    debugFreqAxes = 1e-3*(-settings.acqSearchBand:settings.acqSearchStep:...
        settings.acqSearchBand);% freq serach band [KHz]
end
...
for PRN = settings.acqSatelliteList
    ...
    if debugPlot
        ind = PRN;
        surf(ax_arr{ind},debugTimeAxes,debugFreqAxes,results,'EdgeColor','none');
        title(ax_arr{ind},sprintf('PRN:%02d',PRN),'FontSize',8,'FontWeight','bold');
        view(ax_arr{ind},[90,0])
        if PRN == 1
            xlabel(ax_arr{ind},'Time delay (ms)','FontSize',5);
            ylabel(ax_arr{ind},'Doppler Shift (KHz)','FontSize',5);
        end
    end
